...
async function main() {
  await mongoose.connect(
    "mongodb+srv://musa:5EX1IEKkwojM7RAO@cluster0.h07q12i.mongodb.net/todolistDB"
  );
  ...
  app.post("/", async function (req, res) {
    //The reason we use async function, inside function we use await
  ...
  app.post("/delete", async function (req, res) {
    ...
    if (listName === "Today") {
      ...
      await List.findOneAndUpdate(
        { name: listName }, // WE use mongogse method called pulle here but we can use for each and check remore
